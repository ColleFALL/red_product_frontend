...
export default function HotelCreate() {
  ...
  const { id } = useParams(); // ✅ si présent => edit
  const isEdit = !!id;
  ...
  // ✅ En mode edit : pré-remplir depuis l’API
  useEffect(() => {
    if (!isEdit) return;
    ...
    const fetchHotel = async () => {
      try {
        ...
        // ✅ remplir form (photoPreview optionnel)
        setForm((p) => ({
          ...p,
          nom: data?.nom || "",
          ...
          photo: null, // on ne met pas un fichier ici
          photoPreview: data?.photo
            ? (data.photo.startsWith("http") ? data.photo : `${API}${data.photo}`)
            : "",
        }));
      } catch (err) {
        console.error(err);
